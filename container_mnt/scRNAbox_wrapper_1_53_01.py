...
for step in args.steps.split(','):
    ...
    step = step.strip()
    print("Configuring THREADS_ARRAY for step " + step)
    if step == '5' and args.par_integrate_seurat:
        ...
        if step in threads_array_steps:
            ...
            print("Warning: Step " + step + " is not recognized. Skipping THREADS_ARRAY entry for this step.")
...
if '7' in args.steps.split(','):
    if not any([args.markergsea, args.knownmarkers, args.referenceannotation, args.annotate]):
        print("Error: When selecting step 7, at least one of these options must be set: --markergsea, --knownmarkers, --referenceannotation, --annotate")
        sys.exit(1)
...
for step in args.steps.split(','):
    ...
    if not os.path.exists(step_dir_path):
        print(f"Error: {step_dir_path} does not exist. Please check the steps you provided.")
    ...
    # Verify if at least one file exists in the step director
        ...
        print(f"Number of files in {step_dir_path}: {file_count}")
        if file_count == 0:
            print(f"Error: {step_dir_path} is empty. Please check the steps you provided.")
            execution_failed = True
    ...
    if len(last_log_files)  > 0:
        ...
        if tail_content.returncode == 0:
            ...
            print(f"Error while reading log file {last_log_file}: {tail_content.stderr}")
    else:
        print("No log files found.")
        sys.exit(1)

    if execution_failed:
        print("Execution failed. Please check the steps you provided and the log files.")
        sys.exit(1)
    ...
    if '1' in args.steps.split(','):
        ...
        if bam_files:
            ...
            print("Removing bam files from step1.")
            for bam_file in bam_files:
                try:
                    os.remove(bam_file)
                    print(f"Removed {bam_file}")
                except Exception as e:
                    print(f"Error removing {bam_file}: {e}")
